...
- The program sends messages only in reply to the page: info (its build id and state), a report of all values spread over a few frames, apply and revert. Every message also reaches the MIDI port as SysEx, which a DAW can see, so nothing is sent while you play.
...
- Playing costs the same as the optimised script: a repaint is about 1130 instructions (1860 at most) and an MPE key strike about 260. Applying a preview takes about 4200 instructions in one callback, less than the 4600 the script spent at most on every frame before it was optimised.
