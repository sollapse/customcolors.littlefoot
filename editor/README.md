...
- It also takes the script's host messages, `HCOL`, `KCOL`, `OCOL` and `GOCT` (see the main README). A host's colors sit on top of the values and never change them, so the page doesn't see or save them. While host color mode is on, a key or octave the host has colored shows the host's color instead of a preview.
...
- Playing costs the same as the optimised script: a repaint is about 1130 instructions (1860 at most) and an MPE key strike about 260. Applying a preview takes about 4700 instructions in one callback, about the most the script spent on every frame before it was optimised (4600).
