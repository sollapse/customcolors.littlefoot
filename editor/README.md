...
- **Pitch bend and pressure**: keys you hold can follow how far they're bent, how hard they're pressed, or both. The panel chooses which and sets the five colors: bend down, center and up, and no pressure and full. Keys lit by incoming notes keep their own colors.
...
- **Saved on this computer**: the page keeps your current edits in the browser's local storage, along with named sets. **Export file** and **Import file** save and load the same data as JSON. Files and sets from before the pitch bend and pressure colors load with those at their defaults.
...
- A preview writes the 82 values to the program's shared memory (value n at offset n × 4), then sends a message: the program loads them and replies with their sum, which the page checks.
...
- Playing costs the same as the optimised script: a repaint is about 1130 instructions (1860 at most) and an MPE key strike about 260. Applying a preview takes about 4600 instructions in one callback, about the most the script spent on every frame before it was optimised. With pitch bend or pressure colors on, working out a held key's color adds about 450 instructions to each of its bend and pressure events.
